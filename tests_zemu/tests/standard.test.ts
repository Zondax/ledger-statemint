...
describe('Standard', function () {
  ...
  test.each(models)('sign large nomination', async function (m) {
    ...
    try {
      ...
      await sim.compareSnapshotsAndAccept('.', `${m.prefix.toLowerCase()}-sign_large_nomination`, m.name === 'nanos' ? 3 : 3)
  ...
  test.each(models)('set keys', async function (m) {
    ...
    try {
      ...
      await sim.compareSnapshotsAndAccept('.', `${m.prefix.toLowerCase()}-set-keys`, m.name === 'nanos' ? 16 : 13)
